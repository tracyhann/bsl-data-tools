...
| `IRB` | IRB value or combined IRB label, such as `12345-34567`. |
...
6. Upload the zipped local study folder when prompted.
7. Run the workflow cell and review the printed output plus the local `log.md`.
...
python3 scripts/workflows/create_study_folder_gdrive/run.py \
  --stage all \
  --study-folder studies/12345-34567-AURORA \
  --study-name "AURORA" \
  --irb "12345-34567" \
  --template "URL_TO_TEMPLATE_FOLDER" \
  --destination root \
